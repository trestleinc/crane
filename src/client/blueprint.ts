...
export type BlueprintDraft = {
  ...
  input: (name: string, type: string, required: boolean, description?: string) => BlueprintDraft;

  /** Add a NAVIGATE tile */
  navigate: (url: string, options?: { waitUntil?: 'load' | 'domcontentloaded' | 'networkidle'; timeout?: number }) => BlueprintDraft;
  ...
  /** Add an EXTRACT tile */
  extract: (instruction: string, outputVariable: string, schema?: unknown) => BlueprintDraft;
...
function createDraft(
  ...
  const addTile = (
    ...
    const updatedTiles = tiles.map((t) =>
      t.id === lastTile?.id ? { ...t, connections: { ...t.connections, output: id } } : t
    );

    return createDraft(name, description, [...updatedTiles, newTile], inputSchema, tags);
  };
  ...
  return {
    input: (fieldName, fieldType, required, fieldDescription) => {
      ...
      return createDraft(name, description, tiles, [...inputSchema, newField], tags);
    },

    navigate: (url, options) =>
      addTile('NAVIGATE', `Navigate to ${url.slice(0, 30)}${url.length > 30 ? '...' : ''}`, {
        url,
    ...
    type: (instruction, options) =>
      addTile('TYPE', `Type: ${instruction.slice(0, 30)}${instruction.length > 30 ? '...' : ''}`, {
        instruction,
        ...options,
      }),

    click: (instruction) =>
      addTile('CLICK', `Click: ${instruction.slice(0, 30)}${instruction.length > 30 ? '...' : ''}`, {
        instruction,
      }),
    ...
    select: (instruction, value) =>
      addTile('SELECT', `Select: ${value.slice(0, 30)}${value.length > 30 ? '...' : ''}`, {
        instruction,
        value,
      }),
    ...
    tag: (...newTags) => createDraft(name, description, tiles, inputSchema, [...tags, ...newTags]),
...
export const blueprint = {
   ...
   */
  create: (name: string): BlueprintDraft => createDraft(name, undefined, [], [], []),
} as const;
